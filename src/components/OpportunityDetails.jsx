...
const OppotunityDetails = () => {
  ...
  useEffect( async () => {
    const userRef = doc(getFirestore(), "opportunities", `${postId}`);
  ...
  const confirmDeletePost = async () =>{
    await deleteDoc(doc(getFirestore(), "opportunities", `${postToDelete}`));
    setModalIsOpen(false)
    navigate('/')
  }
...
export default OppotunityDetails